...
void SegUTF8(const char *Input_file_path, const char *Output_file_path){
	//初始化分词组件
	if(!NLPIR_Init("../../../",UTF8_CODE))//数据在当前路径下，设置为UTF8编码的分词
	{
		printf("ICTCLAS INIT FAILED!\n");
	...
	NLPIR_FileProcess(Input_file_path, Output_file_path);
	printf("Complete file"+Input_file_path+" !");
...
int main(int argc, char *argv[])
{
	if (argc<3)
	{
		printf("%s input_file_path output_file_path", argv[0]);
		return -1;
	}
	SegUTF8(argv[1], argv[2]);
}
